...
typedef struct {
   ...
   * - F: Estimated distance from the starting z position to what is below the
   * CF
   *
  ...
  // We store as a quaternion to allow easy normalization (in comparison to a
  // rotation matrix), while also being robust against singularities (in
  // comparison to euler angles)
  float q[4];

  // The quad's attitude as a rotation matrix (used by the prediction, updated
  // by the finalization)
  float R[3][3];
...
/*  - Externalization to move the filter's internal state into the external
 * state expected by other modules */
