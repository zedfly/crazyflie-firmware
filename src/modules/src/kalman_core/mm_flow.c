...
void kalmanCoreUpdateWithFlow(kalmanCoreData_t *this,
  ...
  // ~~~ Camera constants ~~~
  // The angle of aperture is guessed from the raw data register and thankfully
  // look to be symmetric
  ...
  // S[KC_STATE_PZ]; dy_g = R[1][0] * S[KC_STATE_PX] + R[1][1] * S[KC_STATE_PY] +
  // R[1][2] * S[KC_STATE_PZ];
  ...
  predictedNX = (flow->dt * Npix / thetapix) *
                ((dx_g * this->R[2][2] / z_g) - omegaFactor * omegay_b);
  measuredNX = flow->dpixelx;

  // derive measurement equation with respect to dx (and z?)
  hx[KC_STATE_Z] =
      (Npix * flow->dt / thetapix) * ((this->R[2][2] * dx_g) / (-z_g * z_g));
  ...
  predictedNY = (flow->dt * Npix / thetapix) *
                ((dy_g * this->R[2][2] / z_g) + omegaFactor * omegax_b);
  measuredNY = flow->dpixely;

  // derive measurement equation with respect to dy (and z?)
  hy[KC_STATE_Z] =
      (Npix * flow->dt / thetapix) * ((this->R[2][2] * dy_g) / (-z_g * z_g));
...
void kalmanCoreUpdateWithFlowUsingF(kalmanCoreData_t *this,
  ...
  // derive measurement equation with respect to dx (not z and f since it caused
  // some bad behaviour)

  hx[KC_STATE_PX] = (Npix * flow->dt / thetapix) * (this->R[2][2] / z_g);
  // hx[KC_STATE_Z] = (Npix * flow->dt / thetapix) * ((this->R[2][2] * dx_g) /
  // (-z_g * z_g)); hx[KC_STATE_F] = - hx[KC_STATE_Z];
  ...
  // derive measurement equation with respect to dy (not z and f since it caused
  // some bad behaviour)
  hy[KC_STATE_PY] = (Npix * flow->dt / thetapix) * (this->R[2][2] / z_g);
  // hy[KC_STATE_Z] = (Npix * flow->dt / thetapix) * ((this->R[2][2] * dy_g) /
  // (-z_g * z_g)); hy[KC_STATE_F] = - hy[KC_STATE_Z];
